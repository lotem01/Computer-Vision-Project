    ...
    <motion.main className="boot" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 1.02 }}>
      ...
      <section className="boot-content">
        <div className="boot-copy">
          ...
          <p>Loading specialized pose networks into memory and calibrating the visual pipeline for zero-delay switching.</p>
        ...
        <div className="body-viz" aria-label={`${ready} of ${total} models ready`}>
          ...
          <svg viewBox="0 0 100 112" role="img">
            ...
            {joints.map(([x, y], index) => <circle key={index} className={index < Math.max(1, ready * joints.length / total) ? 'active' : ''} cx={x} cy={y} r="2.2" />)}
          </svg>
      ...
      <section className="model-boot-list">
        {(data?.models ?? Array.from({ length: total })).map((model, index) => {
        ...
        <AnimatePresence mode="wait">
          {!connected ? <motion.span key="off" initial={{ opacity: 0 }} animate={{ opacity: 1 }}><WifiOff size={14} /> Start the Python backend to continue</motion.span> :
            data?.completed ? <motion.button key="ready" className="primary-button" onClick={onEnter} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>Enter studio <span>↗</span></motion.button> :
              <motion.span key="load" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>Optimizing model {Math.min(total, ready + failed + 1)} of {total}</motion.span>}
        </AnimatePresence>
      </footer>
    </motion.main>
  )
